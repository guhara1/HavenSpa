...
const pricing = {
  ...
  <section class="section section--tight">
    <div class="wrap prose" style="max-width:74ch">
      <h2>코스별 요금 구성</h2>
      <p>간다GO는 60분·90분·120분 세 가지 코스로 운영합니다. 60분 코스(90,000원)는 목·어깨 등 뭉친 부위를 중심으로 컨디션을 정리하는 기본 릴랙스 케어, 90분 코스(150,000원)는 아로마를 포함한 추천 구성으로 전신 밸런스를 함께 살피는 구성, 120분 코스(180,000원)는 전신을 집중적으로 관리하는 프리미엄 케어입니다. 처음 이용하시는 분은 90분 코스를 가장 많이 선택하십니다.</p>
      <h2>요금 기준 안내</h2>
      <p>표기 요금은 경기북부 전 생활권 공통 기준입니다. 자택·호텔·오피스텔·아파트·펜션 등 이용 장소와 관계없이 동일한 코스 기준으로 안내하며, 표기 금액 외 추가 비용은 없습니다. 상위노출·최저가·1위 같은 표현은 사용하지 않으며, 실제 상담 기준 그대로 안내합니다.</p>
      <p>다만 포천·가평·연천·파주 북부처럼 도심에서 거리가 먼 외곽·접경 지역은 이동 거리에 따라 상담 시 이동 기준을 함께 확인합니다. 자세한 내용은 <a href="/gyeonggi-north/check/travel-fee/">외곽 이동 기준</a>에서 확인할 수 있으며, 코스 요금 자체가 달라지는 것은 아닙니다.</p>
      <h2>예약 방법</h2>
      <p>예약은 전화예약 <a href="${SITE.phoneHref}">${SITE.phone}</a> 또는 문의 채널을 통해 방문 주소와 희망 시간, 이용하실 코스를 알려주시면 됩니다. 이용 장소별 확인 사항은 <a href="/gyeonggi-north/use/">이용 장소 안내</a>, 예약 전 준비는 <a href="/gyeonggi-north/check/">예약 전 확인</a>에서 함께 살펴보실 수 있습니다. 생활권별 안내는 <a href="/">경기북부 홈</a>에서 확인할 수 있습니다.</p>
    </div>
  </section>
  ${faqBlock([
    { q: "추가 비용이 있나요?", a: "표기된 코스 기준 요금 외 추가 비용 없이 안내합니다. 지역·시간대·이동 거리에 따라 상담 시 최종 확인됩니다." },
...
// --- 예약 전 확인 (허브) -------------------------------------------------
const checkItems = [
  ["방문 주소 확인", "정확한 도로명·상세 동·호수를 확인합니다.", "/gyeonggi-north/check/address/"],
  ["건물 출입 방식", "공동현관·엘리베이터·출입 방식을 확인합니다.", "/gyeonggi-north/check/building-access/"],
  ["아파트 공동현관 확인", "공동현관 출입 방법과 방문 시간대를 확인합니다.", "/gyeonggi-north/check/apartment-access/"],
  ["호텔·숙소 정책", "체크인 여부와 객실 정책을 확인합니다.", "/gyeonggi-north/check/hotel-policy/"],
  ["펜션·독채 숙소 확인", "주소·주차·야간 출입 가능 여부를 확인합니다.", "/gyeonggi-north/check/pension-policy/"],
  ["외곽 이동비 기준", "포천·가평·연천 등 외곽 이동 기준을 확인합니다.", "/gyeonggi-north/check/travel-fee/"],
  ["예약 가능 시간", "지역별 예약 가능 시간대를 확인합니다.", "/gyeonggi-north/check/time/"],
  ["예약 변경 기준", "변경·취소 기준을 사전에 확인합니다.", "/gyeonggi-north/check/change-policy/"],
  ["고객 유의사항", "이용 전 알아두면 좋은 유의사항을 확인합니다.", "/gyeonggi-north/check/customer-notice/"],
  ["개인정보 처리방침", "최소 정보만 확인하는 처리 기준을 확인합니다.", "/gyeonggi-north/check/privacy/"],
  ["불법·선정적 서비스 불가", "제공하지 않는 서비스 범위를 확인합니다.", "/gyeonggi-north/check/service-policy/"],
];
  ...
  <section class="section section--tight">
    <div class="wrap">
      <div class="grid grid--2">
        ${checkItems.map(([t, d, h]) => `<a class="card card--link" href="${h}"><h3 style="font-size:1.05rem">${t}</h3><p>${d}</p></a>`).join("")}
      </div>
    </div>
  </section>
  <section class="section section--tight">
...
const privacy = {
  ...
  <section class="section section--tight">
    <div class="wrap prose" style="max-width:78ch">
      ...
      <h2>4. 제3자 위탁</h2>
      <p>사이트는 예약 안내 목적 외로 개인정보 처리를 외부에 위탁하지 않습니다. 부득이하게 위탁이 필요한 경우에는 위탁 대상과 범위를 사전에 고지하고 관계 법령에 따라 관리·감독합니다.</p>
      <h2>5. 안전성 확보 조치</h2>
      <p>개인정보에 대한 접근을 최소한의 인원으로 제한하고, 예약 상담 목적이 끝난 정보는 지체 없이 파기합니다. 별도의 회원 가입이나 마케팅 수신 동의를 받지 않으며, 자동 수집 장치(쿠키 등)를 통한 광고 목적의 개인정보 수집은 하지 않습니다.</p>
      <h2>6. 이용자의 권리</h2>
      <p>이용자는 언제든 본인 정보의 확인·수정·삭제 및 처리 정지를 요청할 수 있으며, 요청 시 지체 없이 조치합니다. 요청은 아래 문의 방법으로 접수할 수 있습니다.</p>
      <h2>7. 문의</h2>
...
const servicePolicy = {
  ...
  <section class="section section--tight">
    <div class="wrap prose" style="max-width:78ch">
      ...
      <h2>운영 원칙</h2>
      <p>모든 안내와 상담은 코스별 기준 요금과 건전한 관리 범위 안에서 이루어집니다. 위 기준에 반하는 요청은 정중히 거절되며, 사이트 내 어떠한 콘텐츠에도 선정적 표현을 사용하지 않습니다. 관리사와 이용 고객 모두가 안전하고 존중받는 환경에서 서비스가 진행되도록 하는 것이 기본 원칙입니다.</p>
      <h2>표현·마케팅 기준</h2>
      <p>간다GO는 상위노출 보장, 최저가, 1위, VIP 은밀 서비스 같은 과장·오해를 부르는 표현을 사용하지 않습니다. 후기와 안내는 실제 이용 경험을 바탕으로 하며, 허위 평점이나 과장된 홍보 문구를 게시하지 않습니다. 이는 건전한 운영과 정확한 정보 전달을 위한 방침입니다.</p>
      <p>이용 기준은 <a href="/gyeonggi-north/check/">예약 전 확인</a>에서, 요금은 <a href="/gyeonggi-north/pricing/">요금 안내</a>에서, 고객이 지켜주시면 좋은 사항은 <a href="/gyeonggi-north/check/customer-notice/">고객 유의사항</a>에서 확인할 수 있습니다.</p>
    </div>
  </section>
  `,
};
